...
(function () {
    ...
    const sons = {
        ...
        fimVotacao: function () {
            playTone(523.25, 0.10, 'sine', 0.2, 0.00);
            playTone(659.25, 0.10, 'sine', 0.2, 0.12);
            playTone(783.99, 0.10, 'sine', 0.2, 0.24);
            playTone(1046.50, 0.10, 'sine', 0.2, 0.36);
            playTone(1318.51, 0.22, 'sine', 0.2, 0.50);
        },
    ...
    function renderFim() {
        setText('fim-titulo', 'FIM DA VOTAÇÃO');

        const resumo = el('votos-resumo');
        if (!resumo) return;

        const html = estado.votos.map(function (v) {
            let nomeCandidato = v.candidato ? v.candidato.nome : v.tipo;
            return '<div class="voto-item">' +
                '<span class="voto-cargo-label">' + v.cargo + '</span>' +
                '<span class="voto-candidato-label">' + nomeCandidato + '</span>' +
                '</div>';
        }).join('');

        resumo.innerHTML = html || '<div style="color:var(--urna-text-dim);font-size:9px;text-align:center">Nenhum voto registrado</div>';
    }
